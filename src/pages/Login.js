...
export const Login = () => {
    ...
    async function handleLogin(event){
        ...
        try{
           ...
           data.accessToken ? toast(<div><i className="bi bi-check-circle"></i> Successfully logged in!</div>,{position:"top-right",autoClose: 3000,hideProgressBar: false,closeOnClick: true,pauseOnHover: false, closeButton:true,theme:"colored",className: 'my-custom-toast-login'}) && navigate("/products") : toast.error(data);
        }catch(error){
  ...
  return (
        ...
        <br/>
        <button  className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"><Link  to="/register" className="block   hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">not Registred ?</Link>
</button>
